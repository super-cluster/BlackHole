...
const CATEGORY_COLORS = {
  ...
  'Frameworks':     'var(--color-ocean-light)',
  'Cloud & DevOps': 'var(--color-teal)',
  'Databases':      '#a78bfa',
  'Architecture':   'var(--color-red-light)',
}
...
export default function Skills() {
  ...
  return (
    <section id="skills" className="section skills" ref={ref}>
      <div className="orb orb-ocean" style={{ top: '20%', left: '-8%', opacity: 0.5, animationDelay: '-7s' }} />
      ...
      <div className="container">
        ...
        >
          ...
          <h2 className="section-title">
            My <span className="highlight">weapons of choice</span>
          </h2>
          <p className="section-subtitle">
            Battle-tested tools forged through years of production engineering.
          </p>
        </motion.div>

        <div className="skills__grid">
          {categories.map(([category, items], catIdx) => {
            const color = CATEGORY_COLORS[category] || 'var(--color-gold)'
            return (
              <motion.div
                key={category}
                className="card skills__cat"
                initial={{ opacity: 0, y: 30 }}
                animate={inView ? { opacity: 1, y: 0 } : {}}
                ...
                <div className="skills__pills">
                  {items.map((skill, si) => (
                    <motion.span
                      ...
                      transition={{ duration: 0.3, delay: 0.25 + catIdx * 0.08 + si * 0.05 }}
        ...
        {/* Weapon rack strip */}
        <motion.div
          className="skills__rack"
          initial={{ opacity: 0, y: 20 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6, delay: 0.7 }}
        >
          <div className="skills__rack-label">⚔️ FULL ARSENAL</div>
