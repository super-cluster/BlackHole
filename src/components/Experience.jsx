...
export default function Experience() {
  ...
  return (
    <section id="experience" className="section experience" ref={ref}>
      <div className="orb orb-violet" style={{ bottom: '-15%', left: '-8%', opacity: 0.5, animationDelay: '-5s' }} />

      <div className="container">
        <motion.div className="exp__header" initial={{ opacity:0, y:30 }} animate={inView ? { opacity:1, y:0 } : {}} transition={{ duration:0.65 }}>
          <span className="section-label">Orbital Records</span>
          <h2 className="section-title">
            Where I&apos;ve <span className="highlight">orbited &amp; delivered</span>
          </h2>
          <p className="section-subtitle">
            Charting the orbital path through enterprise hospitality-tech — shipping production-grade systems at warp speed.
          </p>
        </motion.div>
            ...
            <motion.div key={job.id} className={`exp__job${job.active ? '' : ' exp__job--inactive'}`} initial={{ opacity:0, y:40 }} animate={inView ? { opacity:1, y:0 } : {}} transition={{ duration:0.65, delay:0.12 + jobIdx * 0.1 }}>
              ...
              <div className="exp__log-grid">
                {job.highlights.map((h, idx) => (
                  <motion.div key={idx} className="card exp__log-entry" initial={{ opacity:0, y:18 }} animate={inView ? { opacity:1, y:0 } : {}} transition={{ duration:0.5, delay:0.3 + idx * 0.06 }}>
                    <div className="exp__log-number">{String(idx+1).padStart(2,'0')}</div>
